...
def create_label(epoch, predict_file,prob=0.9):
    ...
    predict_data = getPredictData("../data", predict_file, bTokenizer)
    ...
    for step, batch in enumerate(tqdm(test_dataloader, desc=" eval Iteration")):
        ...
        # print(output.tolist())
        # print(output)
        _, selected_index = ToolUtils.getPseudoIndex(np.array(output.tolist()), ver=prob)
...
def getTrainData(path, name, tokenizer):
    ...
    if args.is_use_psdo:
        ...
        train_segment_ids_list = list(train_segment_ids)
        train_label_ids_list = list(train_label_ids)
        train_input_masks_list = list(train_input_masks)
        psdo_input_id_list, psdo_input_mask_list, \
        psdo_segment_id_list, psedo_label_list = get_psedo_data()

        train_id_list.extend(psdo_input_id_list)
        train_input_masks_list.extend(psdo_input_mask_list)
        train_segment_ids_list.extend(psdo_segment_id_list)
        train_label_ids_list.extend(psedo_label_list)

        train_ids = np.array(train_id_list)
        train_segment_ids =np.array(train_segment_ids_list)
        train_label_ids =np.array(train_label_ids_list)
        train_input_masks =np.array(train_input_masks_list)

    print(np.array(train_ids).shape)

    if not args.is_full_data:
        ...
        all_train_input_ids = torch.tensor(input_ids_array, dtype=torch.long)
        all_train_segment_ids = torch.tensor(segment_ids_array, dtype=torch.long)
        all_train_label_ids = torch.tensor(label_ids_array, dtype=torch.long)
        all_train_input_masks = torch.tensor(input_masks_array, dtype=torch.long)
